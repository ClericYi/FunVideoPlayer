...
int AccompanyDecoder::getMusicMeta(const char* fileString, int * metaData) {
	...
	destroy();
	metaData[0] = sampleRate; // 采样率
	metaData[1] = bitRate; // 比特率
	return 0;
}
...
//	av_register_all(); // 和上方函数一样用于注册编码和解码器，但是已经被废弃，暂时还没在官方Demo中看到替代方案
	...
	// 三个属性用于控制，应该本身存在默认值，暂时没找到
	// 问题：书上说数值越小，解码速度越快，但是具体的控制方案是怎样的？
//	avFormatContext->probesize = xx;
//  avFormatContext->fps_probe_size = xx;
...
//				LOGI(" avcodec_decode_audio4 ...", duration);
				...
				// 这里的gotframe肯定是非零的才会执行，应该是在avcodec_decode_audio4进行了处理
				if (gotframe) {
					int numChannels = OUT_PUT_CHANNELS; // 2
					int numFrames = 0;
					void * audioData;
					if (swrContext) {
						// 帧重采样过，需要用特殊方法处理
						...
						// 并非是AV_SAMPLE_FMT_S16格式的文件，直接废弃，跳出循环
						// 说明这个文件是缺失或者存在各种问题
						if (avCodecContext->sample_fmt != AV_SAMPLE_FMT_S16) {
